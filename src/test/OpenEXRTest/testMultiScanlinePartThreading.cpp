...
public:
    WritingTask (TaskGroup* group, OutputPart& part, int outputLines)
        : Task (group), part (part), outputLines (outputLines)
    {}
...
public:
    ReadingTask (TaskGroup* group, InputPart& part, int startPos)
        : Task (group), part (part), startPos (startPos)
    {}
...
generateFiles (int pixelTypes[], const std::string& fn)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        setOutputFrameBuffer (
            frameBuffer, pixelTypes[i], uintData, floatData, halfData, width);
    ...
    for (int i = 0; i < 2; i++)
    {
        switch (pixelTypes[i])
        {
            ...
            case 1:
                ...
                break;
            case 2: assert (checkPixels<half> (hData[i], width, height)); break;
        }
    }
}
