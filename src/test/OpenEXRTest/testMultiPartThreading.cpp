...
struct WritingTaskData
{
    ...
    WritingTaskData (int partNumber) : partNumber (partNumber) {}
...
generateRandomHeaders (
    int partCount, vector<Header>& headers, vector<WritingTaskData>& taskList)
{
    headers.clear ();
...
readPartialFiles (int randomReadCount, const std::string& fn)
{
    ...
    {
        threadPool->addTask ((new RandomReadingTask (&taskGroup, &file)));
    }
