...
#else
static void reconstruct( uint8_t *buf, size_t sz )
{
    uint8_t *t = buf + 1;
    uint8_t *stop = buf + sz;
    while ( t < stop )
...
static int undozip( const void *src, size_t packsz, void *out, size_t outsz )
{
...
static int undorle( const void *src, size_t packsz, void *out, size_t outsz )
{
