...
struct _internal_exr_seq_scratch_t
{
    uint8_t* scratch;
    uint64_t curpos;
    int64_t  navail;
    ...
    int (*sequential_read) (struct _internal_exr_seq_scratch_t*, void*, uint64_t);
...
static int
scratch_seq_read (struct _internal_exr_seq_scratch_t* scr, void* buf, uint64_t sz)
{
    uint8_t* outbuf  = buf;
    uint64_t nCopied = 0;
    uint64_t notdone = sz;
...
static int
priv_init_scratch (
    exr_file_t* file, struct _internal_exr_seq_scratch_t* scr, uint64_t offset)
{
    struct _internal_exr_seq_scratch_t nil = { 0 };
    *scr                                   = nil;
...
static void
priv_destroy_scratch (struct _internal_exr_seq_scratch_t* scr)
{
    if (scr->scratch) priv_free (scr->scratch);
}
...
static int
read_text (
    struct _internal_exr_context_t*     file,
    char                                text[256],
    int32_t*                            outlen,
    int32_t                             maxlen,
    struct _internal_exr_seq_scratch_t* scratch,
    const char*                         type)
{
    char    b;
    int     rv;
    ...
    int32_t                             attrsz)
{
    ...
    {
        chlen = 0;
        if (read_text (f, chname, &chlen, maxlen, scratch, aname)) return -1;
        attrsz -= chlen + 1;
        ...
        if (scratch->sequential_read (scratch, &ptype, 4)) return -1;
        if (scratch->sequential_read (scratch, &flags, 4)) return -1;
        if (scratch->sequential_read (scratch, &xsamp, 4)) return -1;
        if (scratch->sequential_read (scratch, &ysamp, 4)) return -1;
    ...
    uint8_t                             maxval)
{
    ...
    if (scratch->sequential_read (scratch, attrdata, sizeof (uint8_t)))
        return EXR_GETFILE (f)->print_error (
            f, EXR_ERR_READ_IO, "Unable to read '%s' %s data", aname, tname);
...
{
    int32_t n =
        check_bad_attrsz (f, attrsz, (int) sizeof (float), aname, tname);
    if (n > 0)
    {
        if (exr_attr_float_vector_init (f, attrdata, n)) return 1;

        if (scratch->sequential_read (
                scratch, (void*) attrdata->arr, (uint64_t) attrsz))
        {
    ...
    int32_t                             attrsz)
{
    ...
    while (pulled < attrsz)
    {
        ...
        if (nalloced == 0)
        {
            ...
            if (clist == NULL)
            {
                rv = EXR_GETFILE (f)->print_error (
                    ...
                    "Attribute '%s': Unable to create channel list of size 4",
                    aname);
                goto extract_string_vector_fail;
            }
            nalloced = 4;
        ...
        if ((nstr + 1) >= nalloced)
        {
            ...
            if (nlist == NULL)
            {
                rv = EXR_GETFILE (f)->print_error (
                    f,
                    EXR_ERR_OUT_OF_MEMORY,
                    "Attribute '%s': Unable to create memory for string list of size (%d)",
                    aname,
                    nalloced);
                goto extract_string_vector_fail;
            }
        ...
        *nlist = nil;
        ...
        if (rv != 0) goto extract_string_vector_fail;

        if (scratch->sequential_read (
                scratch, (void*) nlist->str, (uint64_t) nlen))
        {
            rv = EXR_GETFILE (f)->print_error (
                f,
                EXR_ERR_READ_IO,
    ...
    int32_t                             attrsz)
{
    ...
    if (exr_attr_opaquedata_init (f, attrdata, (uint64_t) attrsz)) return 1;
    ...
    int32_t                             attrsz)
{
    ...
    if (exr_attr_preview_init (f, attrdata, sz[0], sz[1])) return 1;

    if (scratch->sequential_read (
            scratch, (void*) attrdata->rgba, sz[0] * sz[1] * 4))
        return EXR_GETFILE (f)->print_error (
            f,
            EXR_ERR_READ_IO,
...
check_populate_channels (
    exr_file_t*                         f,
    struct _internal_exr_part_t*        curpart,
    struct _internal_exr_seq_scratch_t* scratch,
    const char*                         tname,
    int32_t                             attrsz)
{
    ...
    rv = extract_attr_chlist (
        f, scratch, &(tmpchans), EXR_REQ_CHANNELS_STR, tname, attrsz);
    if (rv != 0) return -1;
    ...
    int32_t                             attrsz)
{
    ...
    rv = extract_attr_32bit (
        f, scratch, &(tmpdata), EXR_REQ_DATA_STR, tname, attrsz, 4);
    if (rv != 0) return -1;
    ...
    int32_t                             attrsz)
{
    ...
    rv = extract_attr_32bit (
        f, scratch, &(tmpdata), EXR_REQ_DISP_STR, tname, attrsz, 4);
    if (rv != 0) return -1;
    ...
    int32_t                             attrsz)
{
    ...
    rv = exr_attr_string_init_static_with_length (
        f, curpart->name->string, (const char*) outstr, attrsz);
    if (rv != 0)
    {
        exr_attr_list_remove (f, &(curpart->attributes), curpart->name);
        curpart->name = NULL;
...
check_populate_type (
    ...
    int32_t                             attrsz)
{
    struct _internal_exr_context_t* file = EXR_GETFILE (f);
    int                             rv;
    uint8_t*                        outstr;
    ...
    rv = exr_attr_string_init_static_with_length (
        f, curpart->type->string, (const char*) outstr, attrsz);
    if (rv != EXR_ERR_SUCCESS)
    {
        exr_attr_list_remove (f, &(curpart->attributes), curpart->type);
        curpart->type = NULL;
    ...
    int32_t                             attrsz)
{
    ...
    if (attrsz != 1)
        return -EXR_GETFILE (f)->print_error (
            f, EXR_ERR_INVALID_ATTR, "Invalid version %d: expect 1", attrsz);
        ...
        case 'c':
            if (0 == strcmp (aname, EXR_REQ_CHANNELS_STR))
                return check_populate_channels (
                    file, curpart, scratch, tname, attrsz);
    ...
    struct _internal_exr_seq_scratch_t* scratch)
{
    ...
    if (rv > 0)
    {
        ...
        {
            rv = exr_attr_list_add_by_type (
                f, &(curpart->attributes), name, type, 0, NULL, &nattr);
        }
        if (rv != EXR_ERR_SUCCESS)
            return EXR_GETFILE (f)->print_error (
                f,
        ...
        switch (nattr->type)
        {
            case EXR_ATTR_BOX2I:
                rv = extract_attr_32bit (
                    f, scratch, nattr->box2i, name, type, attrsz, 4);
                break;
            case EXR_ATTR_BOX2F:
                rv = extract_attr_32bit (
                    f, scratch, nattr->box2f, name, type, attrsz, 4);
                break;
            case EXR_ATTR_CHLIST:
                rv = extract_attr_chlist (
                    f, scratch, nattr->chlist, name, type, attrsz);
                break;
            case EXR_ATTR_CHROMATICITIES:
                rv = extract_attr_32bit (
                    f, scratch, nattr->chromaticities, name, type, attrsz, 8);
                break;
            ...
            case EXR_ATTR_DOUBLE:
                rv = extract_attr_64bit (
                    f, scratch, &(nattr->d), name, type, attrsz, 1);
                break;
            case EXR_ATTR_FLOAT:
                rv = extract_attr_32bit (
                    f, scratch, &(nattr->f), name, type, attrsz, 1);
                break;
            case EXR_ATTR_FLOAT_VECTOR:
                rv = extract_attr_float_vector (
                    f, scratch, nattr->floatvector, name, type, attrsz);
                break;
            case EXR_ATTR_INT:
                rv = extract_attr_32bit (
                    f, scratch, &(nattr->i), name, type, attrsz, 1);
                break;
            case EXR_ATTR_KEYCODE:
                rv = extract_attr_32bit (
                    f, scratch, nattr->keycode, name, type, attrsz, 7);
                break;
            case EXR_ATTR_M33F:
                rv = extract_attr_32bit (
                    f, scratch, nattr->m33f->m, name, type, attrsz, 9);
                break;
            case EXR_ATTR_M33D:
                rv = extract_attr_64bit (
                    f, scratch, nattr->m33d->m, name, type, attrsz, 9);
                break;
            case EXR_ATTR_M44F:
                rv = extract_attr_32bit (
                    f, scratch, nattr->m44f->m, name, type, attrsz, 16);
                break;
            case EXR_ATTR_M44D:
                rv = extract_attr_64bit (
                    f, scratch, nattr->m44d->m, name, type, attrsz, 16);
                break;
            case EXR_ATTR_PREVIEW:
                rv = extract_attr_preview (
                    f, scratch, nattr->preview, name, type, attrsz);
                break;
            case EXR_ATTR_RATIONAL:
                rv = extract_attr_32bit (
                    f, scratch, nattr->rational, name, type, attrsz, 2);
                break;
            ...
            case EXR_ATTR_STRING_VECTOR:
                rv = extract_attr_string_vector (
                    f, scratch, nattr->stringvector, name, type, attrsz);
                break;
            case EXR_ATTR_TILEDESC:
                rv = extract_attr_tiledesc (
                    f, scratch, nattr->tiledesc, name, type, attrsz);
                break;
            case EXR_ATTR_TIMECODE:
                rv = extract_attr_32bit (
                    f, scratch, nattr->timecode, name, type, attrsz, 2);
                break;
            case EXR_ATTR_V2I:
                rv = extract_attr_32bit (
                    f, scratch, nattr->v2i->arr, name, type, attrsz, 2);
                break;
            case EXR_ATTR_V2F:
                rv = extract_attr_32bit (
                    f, scratch, nattr->v2f->arr, name, type, attrsz, 2);
                break;
            case EXR_ATTR_V2D:
                rv = extract_attr_64bit (
                    f, scratch, nattr->v2d->arr, name, type, attrsz, 2);
                break;
            case EXR_ATTR_V3I:
                rv = extract_attr_32bit (
                    f, scratch, nattr->v3i->arr, name, type, attrsz, 3);
                break;
            case EXR_ATTR_V3F:
                rv = extract_attr_32bit (
                    f, scratch, nattr->v3f->arr, name, type, attrsz, 3);
                break;
            case EXR_ATTR_V3D:
                rv = extract_attr_64bit (
                    f, scratch, nattr->v3d->arr, name, type, attrsz, 3);
                break;
            case EXR_ATTR_OPAQUE:
                rv = extract_attr_opaque (
                    f, scratch, nattr->opaque, name, type, attrsz);
                break;
...
static int
compute_tile_information (exr_file_t* f, struct _internal_exr_part_t* curpart)
{
    int rv = 0;
...
static int32_t
compute_chunk_offset_size (struct _internal_exr_part_t* curpart)
{
    int32_t                  retval       = 0;
    const exr_attr_box2i_t   dw           = curpart->data_window;
...
update_chunk_offsets (
    ...
    struct _internal_exr_seq_scratch_t* scratch)
{
    struct _internal_exr_part_t *curpart, *prevpart;
    int                          rv = EXR_ERR_SUCCESS;

    if (!file->parts) return EXR_ERR_INVALID_ARGUMENT;
    ...
    for (int p = 0; p < file->num_parts; ++p)
    {
        ...
        if (prevpart != curpart)
            curpart->chunk_table_offset =
                ...
                sizeof (uint64_t) * prevpart->chunk_count;
        prevpart = curpart;
    }
    return rv;
}
...
exr_result_t
internal_exr_parse_header (struct _internal_exr_context_t* file)
{
    struct _internal_exr_seq_scratch_t scratch;
    struct _internal_exr_part_t*       curpart;
    ...
    {
        file->report_error (
            file, EXR_ERR_READ_IO, "Unable to read magic and version flags");
        priv_destroy_scratch (&scratch);
        return rv;
    }
    ...
    if (!curpart)
    {
        rv = file->report_error (
            file, EXR_ERR_INVALID_ARGUMENT, "Error during file initialization");
        priv_destroy_scratch (&scratch);
        return rv;
    }

    file->is_singlepart_tiled = (flags & EXR_TILED_FLAG) ? 1 : 0;
    file->max_name_length     = (flags & EXR_LONG_NAMES_FLAG)
                                    ? EXR_LONGNAME_MAXLEN
    ...
    if (file->is_singlepart_tiled)
    {
        if (file->has_nonimage_data || file->is_multipart)
        {
            rv = file->print_error (
                ...
                (int) file->has_nonimage_data,
                (int) file->is_multipart);
            priv_destroy_scratch (&scratch);
            return rv;
        }
        curpart->storage_mode = EXR_STORAGE_TILED;
    ...
    do
    {
        ...
        {
            rv = file->report_error (
                file, EXR_ERR_FILE_BAD_HEADER, "Unable to extract header byte");
            priv_destroy_scratch (&scratch);
            return rv;
        }
        ...
        if (next_byte == '\0')
        {
            ...
            rv = internal_exr_add_part (file, &curpart);
        }
    ...
    if (rv == EXR_ERR_SUCCESS) rv = update_chunk_offsets (file, &scratch);
