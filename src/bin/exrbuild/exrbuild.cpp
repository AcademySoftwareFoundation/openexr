...
copy_tiledimage (
    ...
    int                  inPart,
    int                  outPart,
    const std::string&   inview = std::string (""))
{
    int buffer_size;
    ...
    StringVector views;
    if (hasMultiView (inhdr)) { views = multiView (inhdr); }
    ...
    switch (t.mode)
    {
        ...
        case MIPMAP_LEVELS:
            ...
            y_levels = 1;
            break;
        case RIPMAP_LEVELS: x_levels = t.xSize; y_levels = t.ySize;
    }
    ...
    for (int x_level = 0; x_level < x_levels; x_level++)
    {
        for (int y_level = 0; y_level < y_levels; y_level++)
        {
            ...
            IMATH_NAMESPACE::Box2i dw =
                in.dataWindowForLevel (x_level, actual_y_level);
            ...
            for (ChannelList::ConstIterator i = inhdr.channels ().begin ();
                 ...
                 ++i)
            {
                if (inview == "" || views.size () == 0 ||
                    viewFromChannelName (i.name (), views) == inview)
                {
    ...
    const std::string&   inview = std::string (""))
{
    ...
    int width  = inhdr.dataWindow ().max.x - inhdr.dataWindow ().min.x + 1;
    int height = inhdr.dataWindow ().max.y - inhdr.dataWindow ().min.y + 1;
    ...
    for (int row = inhdr.dataWindow ().min.y; row <= inhdr.dataWindow ().max.y;
         row++)
    {
        in.readPixels (row);
        out.writePixels (1);
    ...
    int                  outPart)
{
    ...
    inFrameBuffer.insertSampleCountSlice (Slice (
        UINT,
        (char*) (countbuf - (y * width + x)),
        sizeof (unsigned int) * 1,       // xStride
        sizeof (unsigned int) * width)); // yStride

    outFrameBuffer.insertSampleCountSlice (Slice (
        UINT,
        (char*) (countbuf - (y * width + x)),
        sizeof (unsigned int) * 1,       // xStride
        sizeof (unsigned int) * width)); // yStride
         ...
         row++)
    {
             ...
             x++)
        {
            ...
            {
                pointer_start[x * channels + i] =
                    &samples[count * channels + i];
            }
            count += count_start[x];
        }
        //
...
main (int argc, char* argv[])
{
    ...
    if (argc < 3)
    {
        cerr
            << argv[0]
            << " takes a collection of EXR images and outputs them as a single multipart EXR\n";
        cerr
            << std::string (strlen (argv[0]), ' ') << " syntax: " << argv[0]
            << "  [input.exr[:partnum[.view]]] [input2.exr[:partnum[.view]]] ... output.exr\n";
        exit (1);
    }
    ...
    for (int i = 0; i < numInputs; i++)
    {
        ...
        if (colon == string::npos) { partnums[i] = 0; }
        else
        {
            ...
            if (dot != string::npos) { views[i] = num.substr (dot + 1); }
        }
        // open and check
        inputs[i] = new MultiPartInputFile (filename.c_str ());
        if (partnums[i] >= inputs[i]->parts ())
        {
            std::cerr << "oops: you asked for part " << partnums[i] << " in "
                      << argv[1 + i] << ", which only has "
                      << inputs[i]->parts () << " parts\n";
            exit (1);
        }
        ...
        if (views[i] != "")
        {
            if (hasMultiView (headers[i]))
            {
                ...
                //
                for (ChannelList::Iterator c = headers[i].channels ().begin ();
                     c != headers[i].channels ().end ();
                     ++c)
                {
                    if (viewFromChannelName (c.name (), multiview) == views[i])
                    {
                        newList.insert (
                            removeViewName (c.name (), views[i]), c.channel ());
                    }
                }
    ...
    MultiPartOutputFile out (
        argv[argc - 1], &headers[0], headers.size (), false, 4);
